...
const char* server_ip = "192.168.1.225"; // IP of PC running the Python bridge
const uint16_t server_port = 4840;

WiFiClient client;

// --- Variables received over TCP
int b_Homing_E = 0, w_Main_EV = 0, b_SingleStep_E = 0, b_Reboot_Valve_E = 0;
int previous_homing_e = 0;
int previous_single_step = 0;
int previous_b_reboot_valve = 0;
...
const int CALIBRATION_DELAY_US = 3000;
const int MIN_DELAY_US = 500;
const int MAX_DELAY_US = 1200;
...
void loop() {
  ...
  if (client.available()) {
    ...
    if (!deserializeJson(doc, line)) {
      ...
      b_Reboot_Valve_E = doc["b_Reboot_Valve_E"].as<int>();

      // --- Reboot handling ---
      if (b_Reboot_Valve_E == 1 && previous_b_reboot_valve == 0) {
        Serial.println("♻️ Reboot requested!");
        delay(200);
        ESP.restart();
      }
      previous_b_reboot_valve = b_Reboot_Valve_E;
      ...
      if (b_Homing_E == 0 && in_calibration_mode) {
        ...
        Serial.print("🔧 Fine adjust: "); Serial.print(fine_adjust_steps); Serial.println(" steps");
...
void runCalibration() {
  ...
  Serial.println("⚙️ Calibration started...");
  ...
  Serial.println("📥 Waiting fine adjustment...");
